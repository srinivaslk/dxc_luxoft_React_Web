...
class HorizontalSidebar extends React.Component {
  constructor(props) {
    ...
    window.history.pushState('', 'dynamic_page_name', localStorage.getItem('dynamic_page_name'));
  ...
  componentDidUpdate(prevProps) {
    ...
    if (this.props.activePath !== prevProps.activePath) {
      ...
      console.log("componentDidUpdate Headermenu");
    url_path = window.location.href;
    let split_path = url_path.split('/');
    localStorage.setItem('dynamic_page_name', split_path[3]);
    this.setState({ page_name:split_path[3] }); 
  ...
  render() {
          return (    
            <div className="horizontal-menu-wrapper" style={{marginTop:'86px', marginBottom:'-224px'}}  >
            
              <div
                className={classnames(
                  "header-navbar navbar-expand-sm navbar navbar-horizontal navbar-shadow margabs",
                  {
                    "navbar-static": this.props.navbarType === "static",
                    "available-top": this.props.navbarType === "sticky",
                    "floating-nav":
